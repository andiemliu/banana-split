...
const TableModal = ({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            axios.put(`http://localhost:3001/api/storeSplitInput/${id}`, {
                checkedItems,
                peopleNamesArr,
            })
            ...
            try {
                ...
                const owedAmounts = peopleNamesArr.map((person, personIndex) =>
                ...
                const collector = peopleNamesArr[
                    peopleNamesArr.length - 1
                ].replace(/\s*\([^)]*\)\s*/, '')
                ...
                for (let i = 0; i < peopleNamesArr.length - 1; i++) {
                    ...
                    await axios.put(
                        `http://localhost:3001/api/updateOwedAmount/`,
                        { id, username, person, collector, amount }
                    )
                }
            } catch (error) {
    ...
    const handleCheckAllChange = () => {
        ...
        data.line_items.forEach((item, itemIndex) => {
            peopleNamesArr.forEach((person, personIndex) => {
                newCheckedItems[`${personIndex}-${itemIndex}`] =
                    newCheckAllState
            })
        })
