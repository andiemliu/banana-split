...
const ProfilesPage = ({ numberOfPeople, handleCardSave }) => {
        ...
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(
                    `http://localhost:3001/api/getOwedAmounts/${id}/${username}`
                )
    ...
    const displayedProfiles = owedAmounts.reduce((accumulator, entry) => {
        ...
        const existingProfile = accumulator.find(
            (profile) => profile.name === person
        )

        if (existingProfile) {
            // If the profile exists, add the new owed amount to others array
            existingProfile.others.push(
                `${person} owes ${collector} $${amount.toFixed(2)}`
            )
        } else {
            // If the profile doesn't exist, create a new profile with the owed amount
            accumulator.push({
                name: person,
                others: [`${person} owes ${collector} $${amount.toFixed(2)}`],
            })
        }
